    ...
    <>
      ...
      <section id="services" className="scroll-mt-24 py-24 sm:py-28">
        <Container>
          ...
          />
          <div className="mx-auto mt-14 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((s, i) => (
              <ServiceTile
                key={s.slug}
                slug={s.slug}
          ...
          <Reveal>
            <div className="mt-12 flex flex-wrap items-center justify-center gap-3">
              {zones.map((z) => (
                <span
                  key={z.name}
                  className="inline-flex items-center gap-2 rounded-full border border-or/20 px-4 py-1.5 text-sm text-blanc/75"
                >
      ...
      {/* 2. Les risques */}
      <section className="border-y border-or/10 bg-noir-soft py-24 sm:py-28">
        <Container>
          <SectionHeading
            ...
            {home.risks.items.map((item, i) => {
              ...
              return (
                <Reveal key={item.title} delay={i * 0.08}>
                  <div className="flex h-full flex-col rounded-xl border border-or/15 bg-noir p-6 transition-colors hover:border-or/40">
        ...
        <Container>
          ...
          <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {home.trust.items.map((item, i) => {
              ...
              return (
                <Reveal key={item.title} delay={i * 0.08}>
                  <div className="flex h-full flex-col items-center rounded-xl border border-or/15 bg-noir p-7 text-center transition-colors hover:border-or/40">
                    <span className="flex h-12 w-12 items-center justify-center rounded-full border border-or/30">
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {home.testimonials.items.map((t, i) => (
              <Reveal key={t.author + i} delay={i * 0.1}>
                <figure className="flex h-full flex-col rounded-xl border border-or/15 bg-noir-soft p-8">
                  <Quote className="h-8 w-8 text-or/50" />
                  <blockquote className="mt-5 flex-1 text-pretty leading-relaxed text-blanc/85">
                    « {t.quote} »
                  </blockquote>
                  <figcaption className="mt-6 border-t border-or/10 pt-5">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {home.process.steps.map((step, i) => (
              <Reveal key={step.title} delay={i * 0.1}>
                <div className="relative h-full rounded-xl border border-or/15 bg-noir p-7">
      ...
      <section className="py-24 sm:py-28">
        <Container className="max-w-3xl">
          ...
          />
          <div className="mt-12 divide-y divide-or/10 border-y border-or/10">
            {faqPreview.map((item) => (
      ...
      {/* 8. Confidentialité */}
      <section className="border-t border-or/15 bg-noir-soft py-20">
